...
| **SKIPPED** | The stop is skipped, i.e., the vehicle will not stop at this stop. Arrival and departure are optional. SKIPPED value is not propagated to later stops in the same trip, i.e., the vehicle will still stop at subsequent stops unless they also have `schedule_relationship: SKIPPED` in their own `stop_time_update`. Delay from previous stop does not propagate through the skipped stop. One must provide a `stop_time_update` with a `delay` or `time` for a subsequent stop if they want propagation to continue.  |
