...
| **stop_id** | [string](https://developers.google.com/protocol-buffers/docs/proto#scalar) | Conditionally required | One | Must be the same as in stops.txt in the corresponding GTFS feed. Either stop_sequence or stop_id must be provided within a StopTimeUpdate - both fields cannot be empty. If both stop_sequence and stop_id are provided, the pair should match one of the pairs defined in trips.txt in the corresponding GTFS feed, except for the case where the given stop is a station that contains one or more stops. This exception happens when location_type=1 for a stop in [stops.txt](/gtfs/spec/en/reference.md#stopstxt) in the corresponding GTFS. For example, in trips.txt, a train is scheduled to visit a train station with stop_id=10 and stop_sequence=20. In [stops.txt](/gtfs/spec/en/reference.md#stopstxt), stop_id=10 has location_type=1 and its child stations are stop_id=11 and stop_id=12. In this case, a pair of stop_id=12 and stop_sequence=20 in stop_time_update is valid. |
