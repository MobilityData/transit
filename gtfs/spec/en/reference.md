...
| `instructions`| Text | Optional | The `instructions` field indicates instructions for the pathway taken from the `from_stop_id` to the `to_stop_id` (E.g. "Go downstairs to the station concourse"). <br> Note that the `instructions` field should only be used in special circumstances where "Follow the signs for `pathways.signposted_as`" does not provide clear enough instructions to the rider. A case of this could be where there are two pathways to the same location, both with the same signage. In this case, `instructions` should be `Turn right into the first pathway to Montmorency` for example.s|
...
| `reversed_instructions` | Text | Optional | Same as the `instructions` field, but when the pathway is used backwards, i.e. from the `to_stop_id` to the `from_stop_id`.|
