...
| `instructions`| Text | Optional | Instructions for a pathway taken from the `from_stop_id` to the `to_stop_id`. This field should only be used in circumstances when describing pathways by signage alone provides ambiguous instructions for the rider. <br><br>_Example: "Go downstairs to the station concourse"_|
...
| `reversed_instructions` | Text | Optional | Functions in the same way as `instructions` except when the pathway is used from the `to_stop_id` to the `from_stop_id`.|
