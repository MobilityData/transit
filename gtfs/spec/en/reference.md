...
| `signposted_as` | Text | Optional | Text from physical signage that is visible to riders when the pathway is taken from `from_stop_id` to `to_stop_id`. The text in this field should appear exactly how it is printed on the physical signage. <br><br> The string can be consumed to provide text directions to users. <br> _Example: "Follow signs to `signposted_as`"._ <br><br> When the physical signage is multilingual, this field may be populated and translated following the example of `stops.stop_name` in the field definition of `feed_info.feed_lang`.|
| `reversed_signposted_as` | Text | Optional | Functions in the same way as `signposted_as` except applies to the physical signage that is visible to riders when the pathway is taken backward, i.e. from `to_stop_id` to `from_stop_id`. |
| `instructions`| Text | Optional | Instructions useful for riders when the pathway is taken from `from_stop_id` to `to_stop_id`. This field can be used for specifying step-by-step text directions for navigating through the station. It may not refer to the physical signage. <br><br>_Example: "Go downstairs to the station concourse"_|
| `tts_instructions` | Text | Optional | Readable version of the `instructions`. See "Text-to-speech field" in the [Term Definitions](#term-definitions) for more.|
| `reversed_instructions` | Text | Optional | Functions in the same way as `instructions` except applies to instructions when the pathway is taken backward, i.e. from `to_stop_id` to `from_stop_id`.|
| `tts_reversed_instructions` | Text | Optional | Readable version of the `reversed_instructions` field. See "Text-to-speech field" in the [Term Definitions](#term-definitions) for more.|
