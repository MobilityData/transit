...
|   |  | | 01:55:00 A.M. => 25:55:00 |
|   |  | | **Note:** Trips that span multiple dates will have stop times greater than 24:00:00. For example, if a trip begins at 10:30:00 p.m. and ends at 2:15:00 a.m. on the following day, the stop times would be 22:30:00 and 26:15:00. Entering those stop times as 22:30:00 and 02:15:00 would not produce the desired results. |  
|  stop_id | ID | **Required** | The **stop_id** field contains an ID that uniquely identifies a stop. Multiple routes may use the same stop. The **stop_id** is referenced from the [stops.txt](#stopstxt) file. If **location_type** is used in [stops.txt](#stopstxt), all stops referenced in [stop_times.txt](#stop_timestxt) must have **location_type** of 0 or 1. A **location_type** of 0 means that vehicles will visit the stop where passengers board or disembark. A **location_type** of 1 means that vehicles will visit the station; visiting a specific stop within the station is not planned but will be determined in GTFS real-time through [stop_time_update](transit/blob/master/gtfs-realtime/spec/en/reference.md#message-stoptimeupdate) where the **stop_id** of the station is replaced by the **stop_id** of a child stop. For example, in [stop\_times.txt](#stop_timestxt), a train is scheduled to visit a station with **stop_id** of 10 and **stop_sequence** of 20. In [stops.txt](#stopstxt), this station has child stops with **stop_id**'s of 11, 12, and 13. In [stop_time_update](transit/blob/master/gtfs-realtime/spec/en/reference.md#message-stoptimeupdate), providing **stop_sequence**:20 and **stop_id**:11 means that the train will visit the child stop with **stop_id**=11 in the station with **stop_id**=10 .<br/><br/>Where possible, **stop_id** values should remain consistent between feed updates. In other words, stop A with **stop_id 1** should have **stop_id 1** in all subsequent data updates. If a stop is not a time point, enter blank values for **arrival_time** and **departure_time**. |  
