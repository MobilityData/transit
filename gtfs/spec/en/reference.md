...
|  `trip_direction_headsign` | Text | Optional | The general name used by the transit system in it's wayfinding system to identify the direction of travel for the trip (e.g. North, Southbound, Loop, Clockwise). <br> <br> A `trip_direction_headsign` must not replicate the values provided in the fields `trip_headsign` and `stop_times.stop_headsign` but must be complementary with these fields (e.g.: `trip_direction_headsign` "Westbound" with `trip_headsign` "Hillsboro"). |
...
|  `stop_direction_headsign` | Text | Optional | The general text used by the transit agency in it's wayfinding system to identify the direction of travel from this `stop_time` to the subsequent one. (e.g. North, Southbound, Loop, Clockwise). This field overrides the default `trips.trip_direction_headsign` and is useful when the direction name changes within a trip. If the direction name displayed is the same for an entire trip, use `trips.trip_direction_headsign` instead. <br><br> A `stop_direction_headsign` value specified for one `stop_time` does not apply to subsequent `stop_times`s in the same trip. If you want to override the `trip_direction_headsign` for multiple `stop_time`s in the same trip, the `stop_direction_headsign` must be repeated in each `stop_time` row.<br><br>A `stop_direction_headsign` must not replicate the values provided in the `trips.trip_headsign` and `stop_times.stop_headsign`, but must instead be complementary with these fields (e.g. `stop_direction_headsign` "Inbound" with `stop_headsign` "Hillsboro"). |
