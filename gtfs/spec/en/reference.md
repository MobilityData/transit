...
|  `platform_code` | Text | Optional | Identifier for a platform within a station (location_type=0) or an entrance (location_type=2) which matches the signage in the station and that is communicated to help orient the rider.
This field should only be a short identifier (eg. "G" or "3"). Words like "platform", "entrance" or "track" (or the feed’s language-specific equivalent) should not be included. This allows feed consumers to easily internationalize and localize this identifier into other languages. |
