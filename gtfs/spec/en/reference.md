...
|  [stops.txt](#stopstxt) | **Required** | Stops where vehicles pick up or drop off riders. Also defines stations and station entrances/exits.  |
...
|  [fare_rules.txt](#fare_rulestxt)  | Optional | Rules to apply fares for rider journeys. |
...
|  Field Name | Type | Required | Description | 
|  ------ | ------ | ------ | ------ |
|  `stop_id` | ID | **Required** | Identifies a stop, station, or station entrance. Stops, stations or station entrances/exits are collectively referred to as locations. Multiple routes may use the same stop. | 
...
When calculating a rider journey, GTFS-consuming applications interpolate transfers based on allowable time and stop proximity. [Transfers.txt](#transferstxt) specifies additional rules and overrides for selected transfers. 
