...
| `signposted_as` | Text | Optional | Text from physical signage that is visible to riders. The text in this field should appear exactly how it is printed on the physical signage.<br><br>_Example: Follow signs to `signposted_as`_<br><br>When the physical signage is multilingual, this field may be populated and translated following the example of `stops.stop_name` in the field definition of `feed_info.feed_lang`.|
| `reversed_signposted_as` | Text | Optional | Functions in the same way as `signposted_as` except when the pathway is taken from the `to_stop_id` to the `from_stop_id`. |
| `instructions`| Text | Optional | Instructions for a pathway taken from the `from_stop_id` to the `to_stop_id`. This field may be used for specifying step-by-step instructions for navigating more complex physical features.<br><br>_Example: "Go downstairs to the station concourse"_|
...
| `reversed_instructions` | Text | Optional | Functions in the same way as `instructions` except when the pathway is taken from the `to_stop_id` to the `from_stop_id`.|
