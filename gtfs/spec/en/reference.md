...
| `reversed_instructions` | Text | Optional | Functions in the same way as `instructions` except applies to instructions when the pathway is taken backward, from `to_stop_id` to `from_stop_id`.|
